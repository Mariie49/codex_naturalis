...
/**
 * Represents a resource cards in the game, which extends the Card class.
 */
public class ResourceCard extends Card {
	...
	/**
	 * Creates a deck of ResourceCards with varying corner symbols and special symbols.
	 * 
	 * This method generates a set of ResourceCards based on the four kingdoms (PLANT, ANIMAL, FUNGI, INSECT).
	 * For each kingdom, it creates cards with different combinations of corner symbols and special symbols.
	 * The cards are shuffled after creation to ensure random distribution within the deck.
	 */
	private void createCards() {
		...
		for (Symbol kingdom : kingdoms) {
			...
			Card fCard = new Card(CardType.RESOURCE, true, firstFrontCorners, backFirstCorners, 0, false, new ArrayList<>()) {
	 ...
	 */
	public static ArrayList<Corner> reorderCorners(ArrayList<Corner> inputList, cards.Corner symbolA, cards.Corner symbolB ) {
		...
		if (!checkCornerPositions(inputList, symbolA, symbolB)) {
			...
			for (cards.Corner corner : inputList) {
	 ...
	 * in the `resourceCardsDeck` ArrayList.
	 */
	private void shuffleDeck() {
	    Collections.shuffle(resourceCardsDeck);
	}
	/**
	 * Draws a random card from the resource card deck.
	 *
	 * This method randomly selects a card from the `resourceCardsDeck`, removes it from the deck,
	 * and returns it. Before returning, it prints the details of both the front and back sides of the card,
	 * then allows the user to choose which side to play the card on.
	 *
	 * @return The randomly drawn ResourceCard.
	 */
	...
	/**
	 * Overrides the chooseSide() method from the base Card class to allow the user to choose
	 * the front or back side of the resource card.
	 *If the user's input is invalid, it defaults to the front side.
	...
	public Card flip(ArrayList <Corner> cornersA,ArrayList <Corner> cornersB, boolean isFront, Symbol kingdom) {
		...
		if (isFront) {
			...
			return new Card (CardType.RESOURCE,isFront ,cornersA, cornersB, 0, HasCenterSymbol, centerSymbols){
	...
	/**
	 * Checks if the deck of resource cards is empty.
	 *
