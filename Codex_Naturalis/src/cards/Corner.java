...
public class Corner {
     ...
     */
	public Corner(CornerState state, Symbol symbol,SpecialSymbol specialSymbol, boolean isFront, Card card) {
		this.state = state;
		this.symbol = symbol;
     ...
     */
	  public void printCorner() {
	    ...
	    System.out.println("Front=> true, Back=> false: " + isFront);
	  }
	  ...
	  /**
	    ...
	    /*
	     * public CornerPosition getPosition() {
	    	//verifica se il corner è associato ad una carta
	        if (card == null) {
	        ...
	        for (int i = 0; i < corners.size(); i++) {
	            if (corners.get(i) == this) { // Confronta i riferimenti
	                return CornerPosition.values()[i]; // Restituisce la CornerPosition corrispondente all'indice
	            }
	        }
