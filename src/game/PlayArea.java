...
public class PlayArea {
						...
						if (card.getRow() == row && card.getColumn() == col) {
							if (i == 0) {
								...
								if (card.hasCenterSymbols()) {
									...
									symbolToPrint = String.format("%-" + dimMaxCard + "s", centerSymbols.trim());
     ...
     * how many times it appears in the corners and centers of the cards in the play area. 
	 * @param symbol
	...
	public int countsSymbolInPlayArea(Symbol symbol) {
        ...
        for (Card card : placedCards) { // Itera su tutte le carte nella PlayArea
        	...
        	if (card.hasCenterSymbols()) {
                for (Symbol centerSymbol : card.getCenterSymbols()) {
